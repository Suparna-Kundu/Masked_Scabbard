...
int crypto_kem_enc(uint8_t *c, uint8_t *k, const uint8_t *pk)
{
    ...
    for(i=0;i<SABER_N/24;i++){// 3*8 where 3->repetition and transfer them in byte have to devide by 8 
        buf[i] = buf[i];
        buf[(SABER_N/24)+i] = buf[i];
        buf[2*(SABER_N/24)+i] = buf[i];
    }
...
int crypto_kem_keypair_sk_masked_HO(sk_masked_s *sksv1, const unsigned char *sk)
{
  ...
  uint32_t i,j,k;
  BS2POLmu(sk, sksv1->s[0]); //sksv is the secret-key
   
    for (j = 0; j < SABER_N ; j += 2) {
        for (k = 1; k < SABER_SHARES; k++) {
            rand = random_uint32();
            sksv1->s[k][j] = (uint16_t)rand;
            sksv1->s[0][j] -= (uint16_t)rand;
            sksv1->s[k][j + 1] = (uint16_t)(rand >> 16);
            sksv1->s[0][j + 1] -= (uint16_t)(rand >> 16);
        }
    }
...
int crypto_kem_dec_masked_HO(unsigned char *k, const unsigned char *c,sk_masked_s *sk)
{
    ...
    unsigned char kr[SABER_SHARES][64],r[SABER_SHARES][32],buf[SABER_SHARES][64],m[SABER_SHARES][32],m1[SABER_SHARES][96]; 
    int j;
    ...
    for (i = 0; i < SABER_SHARES; i++) {    
        memcpy(&m1[i][0], &m[i][0], 32);
        memcpy(&m1[i][32], &m[i][0], 32);
        memcpy(&m1[i][64], &m[i][0], 32);
    }
    ...
    fail = indcpa_kem_enc_cmp_masked_HO(m1, r, pk, c);
    // fail = 1;
