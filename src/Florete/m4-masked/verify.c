...
static void poly_comp_vect(uint32_t t_w[SABER_EP][SABER_SHARES][SABER_N/32],uint16_t u[SABER_SHARES][SABER_N],uint16_t u_prime[SABER_N])
{
	uint16_t i,j,e_u;
	uint16_t x[SABER_N][SABER_SHARES];
	...
	for(i=0;i<SABER_N;i++)
	{
		e_u= ((u_prime[i])<<((SABER_EQ-SABER_EP)))+(1<<(SABER_EQ-SABER_EP));
		
		for(j=0;j<SABER_SHARES;j++)
			x[i][j]=u[j][i]&(SABER_Q-1);
		
		x[i][SABER_SHARES-1]=(x[i][SABER_SHARES-1]-e_u)&(SABER_Q-1);
	}	
...
static void reduced_SecAnd(uint8_t b[SABER_SHARES],uint32_t t_w1[SABER_EP][SABER_SHARES][SABER_N/32], uint32_t t_y1[SABER_ET][SABER_SHARES][SABER_N/32])
{
	uint32_t t_w[SABER_SHARES][SABER_N/32],t_y[SABER_SHARES][SABER_N/32];
	uint16_t j,k,l;
	uint32_t temp1[SABER_SHARES],temp2[SABER_SHARES],temp3[SABER_SHARES];

	// Secure And for SABER_ET bits of message contain share of ciphertext
	...
	// Secure And for SABER_EP bits of key contain share of ciphertext
	for(j=0;j<(SABER_N/32);j++)
	{
		for(k=0;k<SABER_SHARES;k++)
		{
...
// Masked comparison
void masked_comparison_simple(uint8_t b[SABER_SHARES],uint16_t u[SABER_SHARES][SABER_N],uint16_t v[SABER_SHARES][SABER_N],uint16_t u_prime[SABER_N],uint16_t v_prime[SABER_N])
{

	uint32_t t_w1[SABER_EP][SABER_SHARES][SABER_N/32],t_y1[SABER_ET][SABER_SHARES][SABER_N/32];
	...
	// SecAnd on (SABER_EP*SABER_N+SABER_ET*SABER_N) bits
