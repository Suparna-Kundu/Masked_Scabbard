...
static void refresh_4(uint16_t a[],int n)
{
  int i;
  uint32_t rand;
  uint16_t tmp,tmp1;
...
static uint16_t Psi_4(uint16_t x,uint16_t y)
{
  return (x ^ y)-y;
}

static uint16_t Psi0_4(uint16_t x,uint16_t y,int n)
{
  return Psi_4(x,y) ^ ((~n & 1) * x);
}

void copy_4(uint16_t *x,uint16_t *y,int n)
{
  for(int i=0;i<n;i++) x[i]=y[i];
}

static void impconvBA_rec_4(uint16_t *D,uint16_t *x,int n);

void impconvBA_4(uint16_t *D,uint16_t *x,int n)
{
  uint16_t x_ext[n+1];
  copy_4(x_ext,x,n);
  x_ext[n] = 0;
  impconvBA_rec_4(D, x_ext, n);
}
...
{  
  ...
  {
    rand = random_uint32();
    uint16_t r1=rand&0xf;
    uint16_t r2=(rand>>16)&0xf;
    uint16_t y0=(x[0] ^ r1) ^ r2;
    uint16_t y1=x[1] ^ r1;
    uint16_t y2=x[2] ^ r2;
    
    uint16_t z0=y0 ^ Psi_4(y0,y1);
    uint16_t z1=Psi_4(y0,y2);
  ...
  z[0]=Psi0_4(y[0],y[1],n);
  for(int i=1;i<n;i++)
    z[i]=Psi_4(y[0],y[i+1]);

  uint16_t A[n-1],B[n-1];
  impconvBA_rec_4(A,y+1,n-1);
  impconvBA_rec_4(B,z,n-1);
