...
{   
    // for(i=0;i<SABER_SHARES;i++) 
    // 	y_bit[i][1] = y_bit[i][1] ^ y_bit[i][0];
    // y_bit[0][0] = y_bit[0][0] ^ 0xffffffff;

    // for(i=0;i<SABER_SHARES;i++) 
    // 	y_bit[i][2] = y_bit[i][2] ^ y_bit[i][1];
    // y_bit[0][1] = y_bit[0][1] ^ 0xffffffff;
...
static void pack_bitslice_HO(uint32_t x[kappa], uint32_t y[kappa], const uint8_t coins[(SABER_MU*SABER_N/8)])
{
    ...
    for (j = 0; j < 32; j += 4) {
        ...
        x_bit[0] = x_bit[0] | (( coins[i] & 0x01 ) << j );
        x_bit[1] = x_bit[1] | (( (coins[i] >> 1) & 0x01 ) << j );
        x_bit[2] = x_bit[2] | (( (coins[i] >> 2) & 0x01 ) << j );

        y_bit[0] = y_bit[0] | (( (coins[i] >> 3) & 0x01 ) << j );
        y_bit[1] = y_bit[1] | (( (coins[i] >> 4) & 0x01 ) << j );
        y_bit[2] = y_bit[2] | (( (coins[i] >> 5) & 0x01 ) << j );

        x_bit[0] = x_bit[0] | (( (coins[i] >> 6) & 0x01 ) << (j+1) );
        x_bit[1] = x_bit[1] | (( (coins[i] >> 7) & 0x01 ) << (j+1) );
        x_bit[2] = x_bit[2] | (( (coins[i+1] >> 0) & 0x01 ) << (j+1) );

        y_bit[0] = y_bit[0] | (( (coins[i+1] >> 1) & 0x01 ) << (j+1) );
        y_bit[1] = y_bit[1] | (( (coins[i+1] >> 2) & 0x01 ) << (j+1) );
        y_bit[2] = y_bit[2] | (( (coins[i+1] >> 3) & 0x01 ) << (j+1) );

        x_bit[0] = x_bit[0] | (( (coins[i+1] >> 4) & 0x01 ) << (j+2) );
        x_bit[1] = x_bit[1] | (( (coins[i+1] >> 5) & 0x01 ) << (j+2) );
        x_bit[2] = x_bit[2] | (( (coins[i+1] >> 6) & 0x01 ) << (j+2) );

        y_bit[0] = y_bit[0] | (( (coins[i+1] >> 7) & 0x01 ) << (j+2) );
        y_bit[1] = y_bit[1] | (( (coins[i+2] >> 0) & 0x01 ) << (j+2) );
        y_bit[2] = y_bit[2] | (( (coins[i+2] >> 1) & 0x01 ) << (j+2) );

        x_bit[0] = x_bit[0] | (( (coins[i+2] >> 2) & 0x01 ) << (j+3) );
        x_bit[1] = x_bit[1] | (( (coins[i+2] >> 3) & 0x01 ) << (j+3) );
        x_bit[2] = x_bit[2] | (( (coins[i+2] >> 4) & 0x01 ) << (j+3) );

        y_bit[0] = y_bit[0] | (( (coins[i+2] >> 5) & 0x01 ) << (j+3) );
        y_bit[1] = y_bit[1] | (( (coins[i+2] >> 6) & 0x01 ) << (j+3) );
        y_bit[2] = y_bit[2] | (( (coins[i+2] >> 7) & 0x01 ) << (j+3) );
...
static void unpack_bitslice_HO(uint16_t r[32], uint32_t z_bit[lambda])
{
    ...
    for (i = 0; i < 32; i += 2) {
        ...
        z = (( ((z_bit[2] & 0x02) << 1) | ((z_bit[1] & 0x02) << 0) | ((z_bit[0] & 0x02) >> 1)) << 16) | ((z_bit[2] & 0x01) << 2) | ((z_bit[1] & 0x01) << 1) | ((z_bit[0] & 0x01));
    ...
    for (i = 0; i < SABER_L; i++) {
        for (j = 0; j < SABER_N / 32; j++) { //creates 32 samples at a time
            for (l = 0; l < SABER_SHARES; l++) 
                pack_bitslice_HO(x_bit[l], y_bit[l], &coins[l][(i * SABER_POLYCOINBYTES) + (((32 *SABER_MU)/8)* j)]);
