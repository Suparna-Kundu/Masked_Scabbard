...
#define h2 ((1 << (SABER_EP - 5)) - (1 << (SABER_EP - SABER_ET - 1)) + (1 << (SABER_EQ - SABER_EP - 1)))
...
uint32_t InnerProdEnc(uint8_t ct1[SABER_SCALEBYTES_KEM], const uint8_t pk[SABER_INDCPA_PUBLICKEYBYTES], uint16_t sp[SABER_L][SABER_N], const uint8_t m[SABER_KEYBYTES], int compare)
{
    ...
    for (j = 0; j < SABER_N; j++) {
        vp[j] = (vp[j] - (mp[j] << (SABER_EP - 4)) + h1) >> (SABER_EP - SABER_ET);
    }
...
void InnerProdEnc_masked_HO(uint16_t ct1[SABER_SHARES][SABER_N], const uint8_t pk[SABER_INDCPA_PUBLICKEYBYTES], uint16_t sp[SABER_SHARES][SABER_L][SABER_N], const uint8_t m[SABER_SHARES][SABER_KEYBYTES])
{
    ...
    uint16_t mptemp[SABER_N][SABER_SHARES];
    uint16_t mptemp1[SABER_N][SABER_SHARES];
...
// message 4 bit B2A conversion
    for(j=0;j<SABER_N;j++) 
	{	
		for (k=0; k<SABER_SHARES; k++)
		{
			mptemp[j][k]= mp[k][j] & 0x0f;
		}
	}
    for (j = 0; j < SABER_N; j += 1) {
        impconvBA_4(mptemp1[j], mptemp[j], SABER_SHARES);
    }
    for(j=0;j<SABER_N;j++) 
	{	
		for (k=0; k<SABER_SHARES; k++)
    ...
    // uint8_t m1[SABER_SHARES][SABER_KEYBYTES];
    ...
    for (k = 0; k < SABER_SHARES; k++) {
        for (j = 0; j < SABER_N; j++) {
            vp[k][j] = vp[k][j] - (mp[k][j] << (SABER_EP - 4));
            if (k == 0) {
                vp[k][j] += h1;
            }
        }
