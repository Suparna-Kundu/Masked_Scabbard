...
void A2B_bitsliced_msg(uint16_t msg[SABER_SHARES][SABER_N], const uint16_t vp[SABER_N][SABER_SHARES])
{
    ...
    for (size_t i = 0; i < SABER_N/32; i += 1)
    {
        ...
        for (size_t j = 0; j < SABER_SHARES; j++)
        {
            ...
            {
                // msg[j][32*i + k] = 0;
                // for (size_t k1 = 1; k1 < 5; k1++)
                // {
                //     msg[j][32*i + k] |= ((v2_bitsliced[j][SABER_EP - k1]>>k)&1)<<(4-k1);
                // }

                msg[j][32*i + k] = ((((v2_bitsliced[j][SABER_EP - 1]>>k)&1)<<3)|(((v2_bitsliced[j][SABER_EP - 2]>>k)&1)<<2)|(((v2_bitsliced[j][SABER_EP - 3]>>k)&1)<<1)|((v2_bitsliced[j][SABER_EP - 4]>>k)&1));
            }    
        }
    }
}
